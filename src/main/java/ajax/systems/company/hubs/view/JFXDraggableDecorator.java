...
public class JFXDraggableDecorator extends VBox{
     ...
     */
    public JFXDraggableDecorator(Stage stage, Node node) {
     ...
     */
    public JFXDraggableDecorator(Stage stage, Node node, boolean fullScreen, boolean max, boolean min) {
        primaryStage = stage;
    ...
    private void handleDragEventOnDecoratorPane(MouseEvent mouseEvent) {
        ...
        if (!osName.toUpperCase().startsWith("MAC") && (primaryStage.isFullScreen() || mouseEvent.isStillSincePress() || primaryStage.isMaximized() || maximized)) {
            return;
        }
